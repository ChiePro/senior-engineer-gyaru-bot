...
- **発話ゲートの分離(実運用フィードバック反映・構造変更)**: 「1回の回答生成に『答える or `<skip/>`』を
  兼ねさせる」方式は、答えようとするモデルの引力に負けて過剰応答になりがちだった(相槌・他人へのコメント・
  自分の沈黙への言及にまで返す/『スキップした』と内部を実況する)。そこで **「今喋るべきか」の判定を
  回答生成から切り離す**:
  - グループ、または発言が他の人を @ しているときは、まず `strands_runtime.should_speak`(記憶もツールも
    持たない軽い1回の呼び出し、原則 NO に倒した `persona.SPEAK_GATE_PROMPT`)で YES/NO を判定。
    `core.parse_speak_decision` が既定 NO で bool 化。NO なら回答生成すら走らせず黙る。失敗時も黙る(fail-closed)。
  - 純粋な1対1(きあら+発言者だけ・他人言及なし)はゲート無しで毎回返答(チャットとして自然)。
  - 自分の沈黙・skip・内部判断を会話で実況/説明/謝罪しない方針を `persona.BEHAVIOR_GUIDE` に明記(全経路で常時適用)。
  - gpt-oss-120b の実機 probe で、相槌『うん』/他人宛て『坂もっちゃんは何食べたい?』/沈黙へのコメント
    『黙ってくれて嬉しい』/メタ指図『ちゃんと考えろよ』は 0/4 で黙り、明確な質問・呼びかけは YES になることを確認。
