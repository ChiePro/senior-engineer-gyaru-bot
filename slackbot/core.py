...
# 日本標準時。日本は DST が無いので固定 +9 で正しい(tzdata 不要で堅牢)。
JST = timezone(timedelta(hours=9))
_WEEKDAYS_JA = ("月", "火", "水", "木", "金", "土", "日")


def format_clock_note(dt) -> str:
    """タイムゾーン付き datetime を「現在の日時(JST)」の注記文字列にする(純粋・整形のみ)。

    LLM は現在時刻を知らないため、応答ごとにこの注記を system prompt へ注入する。実際の現在時刻の
    取得(.now=副作用)は I/O 層(strands_runtime)で行い、ここには持ち込まない。
    """
    wd = _WEEKDAYS_JA[dt.weekday()]
    return f"現在の日時(JST): {dt.year}年{dt.month}月{dt.day}日({wd}) {dt.hour:02d}:{dt.minute:02d}"
